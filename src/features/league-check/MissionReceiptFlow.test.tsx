...
describe('MissionReceiptFlow — receipt is never gated on 6/6', () => {
  it('A. 0/6, no Calculator session: GENERATE RECEIPT is present and produces a receipt', () => {
    render(<MissionReceiptFlow beforeCheck={null} agent={AGENT} checkedItems={NONE_CHECKED} checkedCount={0} total={6} receiptId="test-receipt-id" />);
  ...
  it('B. 5/6, with a Calculator session: GENERATE RECEIPT is present and produces a receipt', () => {
    render(<MissionReceiptFlow beforeCheck={buildBeforeCheck()} agent={AGENT} checkedItems={FIVE_OF_SIX} checkedCount={5} total={6} receiptId="test-receipt-id" />);
    expect(screen.getByRole('button', { name: /generate receipt/i })).toBeEnabled();
    clickGenerate();
    expect(screen.getByText('Mission Receipt')).toBeInTheDocument();
  });
  ...
  it('F. League Check without a Calculator session at all: the flow renders and the receipt still generates', () => {
    render(<MissionReceiptFlow beforeCheck={null} agent={AGENT} checkedItems={ALL_CHECKED} checkedCount={6} total={6} receiptId="test-receipt-id" />);
    clickGenerate();
    expect(screen.getByText('Mission Receipt')).toBeInTheDocument();
  ...
  it('G. without a Calculator session, Mission Value reads Pending Calculation, not 0', () => {
    render(<MissionReceiptFlow beforeCheck={null} agent={AGENT} checkedItems={FIVE_OF_SIX} checkedCount={5} total={6} receiptId="test-receipt-id" />);
  ...
  it('the button label calls out Mission Approved once 6/6 is reached', () => {
    render(<MissionReceiptFlow beforeCheck={buildBeforeCheck()} agent={AGENT} checkedItems={ALL_CHECKED} checkedCount={6} total={6} receiptId="test-receipt-id" />);
